...
impl MessageHandler<TriggerWalFlush> for WalFlushHandler {
    async fn handle(&mut self, message: TriggerWalFlush) -> Result<()> {
        ...
        // On fencing, close the durable watermark channel so memtable-mode
        // waiters sleeping on `BatchDurableWatcher::wait` return WriterFenced
        // instead of hanging. Keep the dispatcher loop alive so any already-
        // queued triggers still have their `done` cells filled (with fence
        // errors from the appender's `check_fenced`); shutting down here
        // would drop those cells and panic any task waiting on them.
        if is_fenced {
...
mod tests {
    ...
    // Memtable-mode fencing regression: when writer A is fenced mid-flight,
    // its next durable put must surface `WriterFenced` within a bounded time
    // (no hang on the durability watermark) and subsequent puts must fail
    // fast. The first guarantee depends on `WalFlushHandler::handle` calling
    // `WalFlusher::close_watermark()` on a fence error so durable waiters
    // sleeping on `BatchDurableWatcher::wait` get unblocked with
    // `Error::WriterFenced` rather than blocking forever.
    #[tokio::test]
    async fn test_memtable_fenced_durable_put_does_not_hang() {
        let (store, base_path, base_uri, _temp_dir) = create_local_store().await;
        let schema = schema_with_pk();
        let shard_id = Uuid::new_v4();
        ...
        // Writer B claims a higher epoch and writes — fences A in the manifest
        // and on the next-WAL-position slot.
        let writer_b = ShardWriter::open(
            store,
            base_path,
            base_uri,
        ...
        assert!(writer_b.epoch() > writer_a.epoch());
        writer_b
            .put(vec![create_test_batch(&schema, 100, 1)])
            .await
            .unwrap();

        // A's next durable put must fail with WriterFenced within a bounded
        // time. Without close_watermark() the durable waiter would sleep on
        // the watermark channel forever after the WAL flush errored.
        let put_fut = writer_a.put(vec![create_test_batch(&schema, 200, 1)]);
        let result = tokio::time::timeout(std::time::Duration::from_secs(5), put_fut)
            .await
            .expect("fenced durable put hung — close_watermark regression");
        let err = result.expect_err("fenced writer's durable put should fail");
        assert!(
            err.is_writer_fenced(),
            "fenced durable put must surface WriterFenced, got: {err}"
        );

        // Subsequent puts must keep failing (no hang, no panic).
        for i in 0..3 {
            let put_fut = writer_a.put(vec![create_test_batch(&schema, 300 + i, 1)]);
            let result = tokio::time::timeout(std::time::Duration::from_secs(2), put_fut)
                .await
                .expect("post-fence put hung");
            assert!(result.is_err(), "post-fence put #{i} should fail");
        }
