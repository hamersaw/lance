...
impl MemTableFlusher {
    ...
    #[instrument(name = "mt_flush_storage", level = "info", skip(self, memtable), fields(shard_id = %self.shard_id, epoch, generation = memtable.generation(), row_count = memtable.row_count()))]
    ...
    #[instrument(name = "mt_write_data_file", level = "debug", skip(self, memtable), fields(path = %path))]
    ...
    #[instrument(name = "mt_flush_with_indexes", level = "info", skip(self, memtable, index_configs), fields(shard_id = %self.shard_id, epoch, generation = memtable.generation(), row_count = memtable.row_count(), index_count = index_configs.len()))]
